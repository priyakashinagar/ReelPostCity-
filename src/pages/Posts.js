...
function Posts({ onNavigate }) {
  ...
  const PostCard = ({ post, relatedPosts }) => {
    ...
    if (post.type === 'thought') {
      return (
        <div className="bg-gray-800 rounded-xl shadow-lg shadow-blue-400/20 hover:shadow-blue-400/40 transition-all duration-300 border border-gray-700 hover:border-blue-400/50 p-6 mb-6">
          <div className="flex items-center gap-3 mb-4 justify-between">
            <p className="text-xs text-gray-400">📍 {post.city} • {post.date}</p>
            <span className="px-3 py-1 bg-purple-900/50 text-purple-300 text-xs rounded-full font-medium">💭 Thought</span>
          </div>
          
          <h2 className="text-xl font-bold text-gray-100 mb-3">{post.title}</h2>
          <p className="text-gray-300 mb-4 leading-relaxed text-base">{post.description}</p>
          
          <div className="flex gap-2 flex-wrap mb-4">
            {post.tags.map((tag, idx) => (
              <span key={idx} className="text-xs bg-purple-900/30 text-purple-300 px-3 py-1 rounded-full border border-purple-500/30">
                #{tag}
              </span>
            ))}
          </div>
    ...
    if (post.type === 'poll') {
      return (
        <div className="bg-gray-800 rounded-xl shadow-lg shadow-blue-400/20 hover:shadow-blue-400/40 transition-all duration-300 border border-gray-700 hover:border-blue-400/50 mb-6 overflow-hidden">
          <div className="p-6">
            <div className="flex items-center gap-3 mb-4 justify-between">
              <p className="text-xs text-gray-400">📍 {post.city} • {post.date}</p>
              <span className="px-3 py-1 bg-green-900/50 text-green-300 text-xs rounded-full font-medium">📊 Poll</span>
            </div>

            <h2 className="text-lg font-bold text-gray-100 mb-4">{post.title}</h2>
            ...
            <p className="text-xs text-gray-500 text-center mb-4">{totalVotes} total votes</p>
      ...
      return (
        <div className="bg-gray-800 rounded-xl shadow-lg shadow-blue-400/20 hover:shadow-blue-400/40 transition-all duration-300 border border-gray-700 hover:border-blue-400/50 mb-6 overflow-hidden">
          <div className="p-6 pb-0">
            <div className="flex items-center gap-3 mb-4 justify-between">
              <p className="text-xs text-gray-400">📍 {post.city} • {post.date}</p>
              <span className="px-3 py-1 bg-red-900/50 text-red-300 text-xs rounded-full font-medium">🎥 Video</span>
            </div>

            <h2 className="text-lg font-bold text-gray-100 mb-2">{post.title}</h2>
            <p className="text-gray-300 mb-4">{post.description}</p>
          </div>

          <div 
            className="relative w-full bg-gray-900 group max-h-96 overflow-hidden mx-6 rounded-lg mb-4" 
          ...
          >
            ...
            <span className="absolute top-3 right-3 bg-black/70 text-white text-xs font-bold px-2 py-1 rounded">{post.duration}</span>
          </div>

          <div className="p-6 pt-0">
            <div className="flex gap-2 flex-wrap mb-4">
              {post.tags.map((tag, idx) => (
                <span key={idx} className="text-xs bg-red-900/30 text-red-300 px-3 py-1 rounded-full border border-red-500/30">#{tag}</span>
              ))}
            </div>
    ...
    return (
      <div className="bg-gray-800 rounded-xl shadow-lg shadow-blue-400/20 hover:shadow-blue-400/40 transition-all duration-300 border border-gray-700 hover:border-blue-400/50 mb-6 overflow-hidden">
        <div className="p-6 pb-0">
          <div className="flex items-center gap-3 mb-4 justify-between">
            <p className="text-xs text-gray-400">📍 {post.city} • {post.date}</p>
            <span className="px-3 py-1 bg-blue-900/50 text-blue-300 text-xs rounded-full font-medium">🖼 Image</span>
          </div>
        ...
        <img src={post.image} alt={post.title} className="w-full h-96 object-cover mx-6 rounded-lg mb-4" />

        <div className="p-6 pt-0">
          <div className="flex gap-2 flex-wrap mb-4">
            {post.tags.map((tag, idx) => (
              <span key={idx} className="text-xs bg-blue-900/30 text-blue-300 px-3 py-1 rounded-full border border-blue-500/30">
                #{tag}
              </span>
            ))}
          </div>

          <div className="flex gap-6 pt-4 border-t border-gray-700 text-gray-400">
            <button onClick={() => handleLike(post.id)} className={`flex items-center gap-2 hover:text-red-400 transition ${isLiked ? 'text-red-500 font-bold' : ''}`}>
              {isLiked ? '❤️' : '🤍'} {post.likes}
            </button>
            <button className="flex items-center gap-2 hover:text-blue-400 transition">💬 {post.comments ? post.comments.length : 0}</button>
        ...
        {authorRelated.length > 0 && (
          <div className="p-6 pt-4 border-t border-gray-700 bg-gray-750/50">
            <p className="text-xs text-gray-500 mb-3 uppercase font-semibold tracking-wider">More from {post.author}</p>
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
              {authorRelated.map((relPost) => (
                <div key={relPost.id} className="flex-shrink-0 w-32 h-40 bg-gray-700 rounded-lg overflow-hidden cursor-pointer hover:ring-2 hover:ring-blue-500 transition-all group">
                  ...
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    {relPost.type === 'video' && <span className="text-2xl">▶️</span>}
                    {relPost.type === 'thought' && <span className="text-2xl">💭</span>}
                    {relPost.type === 'image' && <span className="text-2xl">🖼</span>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="space-y-4">
            {posts.map((post) => {
              ...
              return (
                <PostCard key={post.id} post={post} relatedPosts={relatedPosts} />
              );
            })}
          </div>
